...
if ((typeName _azi) != (typeName 0)) exitWith {hint format ["Log: [objectMapper] Template azimuth (1) must be a Number! (%1)", _azi]; []};
...
if ((typeName _objs) == (typeName "")) then 
{
	...
	_script = COMP_PATH + _objs + ".sqf" ; //getText(_cfgObjectComps >> _objs >> "objectScript");
	_objs = [];
	_test = "err " + str(_arr);
...
if(isnil "_objs") then
{
diag_log format ["Could not load composition file! '%1' '%2'", _script, _objs];
};
...
for "_i" from 0 to ((count _objs) - 1) do
{
	...
	if ((random 1) > _rdm) then 
	{
		...
		/// _type = [_type,_side] call replaceStatic;
		
		if(_type != "") then // Some objects cant be replaced
		{
		
		/// _type = _type call replaceObjectColor; // Change building/sandbag color
		...
		//if(!isnil "compStick") then
		//{
		[_newObj,_newPos] spawn
		{
		 params ["_newObj","_newPos"];
		...
		*/
		...
		//diag_log format["Placing %1", _newObj];
		...
		_guardPosts = _guardPosts + [[_relPos,_azimuth]];
		};
		
	};
};

[_newObjs,_guardPosts]